...
public class SanDbResult<T> {
    ...
    public static <T extends SanDbResult<T>> List<T> getAllDataByQuery(Class<?> mClass, String whereQuery, String[] whereArgs, int limit, int offset) {
        Table table = SanDBHandler.mTable(mClass);
        return CreateDatabase.getInstance().getAllData(mClass, table.getTbName(), whereQuery, whereArgs, limit, offset);
    }
